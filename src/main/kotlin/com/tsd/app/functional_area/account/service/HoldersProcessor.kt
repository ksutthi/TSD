package com.tsd.app.functional_area.account.service
...
class HoldersProcessor(
    ...
    private val identityManager: RegistryIdentityManager
) {
            ...
            lines.drop(1).forEach { line ->
                ...
                if (cols.size >= 7) {
                    try {
                        ...
                        val name = cols[3]
                        val addr = cols[4]
                        val hash = hexStringToByteArray(cols[5])
                        val balance = cols[6].toBigDecimal()

                        // 1. IDENTITY LINKING
                        val link = identityManager.linkIdentity(pId, accNo, insId, hash)
